      ...
      searchFilter=(text)=>{
        if(text){
          const newData=this.state.univercities.filter((item)=>{
            const itemData=item.univercityName ? item.univercityName.toUpperCase()
            :''.toUpperCase();
    ...
    render(){
      ...
      <View > 
         <Card           style={{width:100,height:100}}
                        move="fadeInUp" 
                        image={{ uri: `http://192.168.2.199:45455/${item.univercityImage}`}} 
                        title={item.univercityName}
                        subtitle={item.univercityRector}
                        
                        screenchange = {()=>this.props.navigation.navigate('Detail',{univercityId:item.univercityId})}
                        /> 
  </View>
)}/>
